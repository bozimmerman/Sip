			...
			&&(!sprops.internal.toUpperCase().startsWith("N")))
			{
				...
				if(isModify)
				{
					...
					for(var i=0; i<newContainerDiv.children.length; i++)
					{
						...
						if(child.style.height === '20px' || child.style.height === '0px')
							titleBar=child;
					}
					containerDiv=newContainerDiv.parentNode;

					// find siblings that need updating
					var siblingsToUpdate=[];
					for(var i=0; i<containerDiv.children.length; i++)
					{
						if(containerDiv.children[i] !== newContainerDiv)
							siblingsToUpdate.push(containerDiv.children[i]);
					}
				...
				else
				{
					...
					newContainerDiv.style.position = 'absolute';
					...
					newContentWindow.style.position = 'absolute';
